...
data class DailyResponse(val status: String, val result: Result) {
    ...
    data class Result(var daily: Daily)

    data class Daily(var temperature: List<Temperature>, val skycon : List<Skycon>,
                     @SerializedName("life_index") val lifeIndex: LifeIndex)

    data class Temperature (val max: Float, var min: Float)
